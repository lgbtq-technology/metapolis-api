...
tap.test('wiki handler works', async t => {
  ...
  await fs.mkdirsAsync(path.resolve(root, 'TTEST', 'CTEST', '_wiki'));

  await fs.writeJsonAsync(path.resolve(root, 'TTEST', 'CTEST', '_wiki', 'index.md'), {
    title: 'test',
    content: 'test'
  });
